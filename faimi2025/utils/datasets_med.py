...
class SubpopDataset:
    ...
    def __init__(self, root, split, metadata_path, transform, train_attr='yes', subsample_type=None, duplicates=None,
                 augmentation=False, stage=None, dynamic_num_samples=False, pre_extracted_feats=None,
                 transform_eval=None, multiclass=False, *args, **kwargs):
...
class HAM10000(SubpopDataset):
    ...
    def __init__(self, root, split, train_attr='yes', subsample_type=None, duplicates=None,
                 augmentation=False, stage=None, transform_eval=None, **kwargs):
        _root = root.copy()
        root = os.path.join(_root, "isic_2018", "ISIC2018_Task3_Training_Input")
        # metadata = os.path.join(data_path, "isic_2018", "metadata", "HAM10000_metadata_subpop_rs0.csv")
        metadata = os.path.join(_root, "isic_2018", "metadata", "HAM10000_metadata_subpop.csv")
...
class ISIC2018(SubpopDataset):
    ...
    def __init__(self, root, split, train_attr='yes', subsample_type=None, duplicates=None,
        ...
        root = os.path.join(_root, "isic_2018", f"ISIC2018_Task3_{target_folder}_Input")
        metadata = os.path.join(_root, "isic_2018", "metadata", "isic2018_metadata_subpop.csv")
        self.data_type = "images"
        if augmentation:
            transform = transforms.Compose([
                v2.RandomHorizontalFlip(p=0.5),
...
class BK(SubpopDataset):
    ...
    def __init__(self, root, split, train_attr='yes', subsample_type=None, duplicates=None,
        ...
        if metadata_path is None:
            metadata_path = os.path.join(root, "bk_metadata_subpop.csv")
        self.data_type = "images"
        ...
        rf_a_path = f'{root}/{split_map[split]}/rf_a.npy'
    ...
    def __getitem__(self, i):
        ...
        if self.feats is not None:
            ...
            x = self.transform(self.x[i])
        return i, x, y, a, torch.tensor([-1]), feat
